...
def fetch_github_stats(repos):
    ...
    # Commits this year via search API
    ...
    # Contributed to (repos user pushed to but doesn't own)
    contrib_resp = requests.get(
        f"https://api.github.com/search/repositories?q=user:{USERNAME}+fork:false&per_page=1",
        headers=HEADERS
    )
    # approximate: count repos where user has contributions via events
